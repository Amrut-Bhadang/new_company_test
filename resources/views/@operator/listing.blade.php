...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor">{{ __('Operator Manager') }}</h4>
    </div>
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ __('Home') }}</a></li>
                <li class="breadcrumb-item active">{{ __('Operator Manager') }}</li>
            </ol>
            @can('Operator-create')

              @if($user_type == 4)
                <a href="#" class="btn btn-primary d-none d-lg-block m-l-15" title="{{ __('Add Operator') }}" data-toggle="modal" data-target="#add_modal" ><i class="fa fa-plus"></i> {{ __('Add Operator') }}</a>
              @endif
               <!-- <a href="{{ url('/operator/exportUsers/0') }}" class="btn btn-warning d-none d-lg-block m-l-15" title="{{ __('Export') }}"><i class="fa fa-download"></i> {{ __('Export') }}</a> -->
            @endcan
        </div>
    </div>
</div>
    ...
    <div class="row">
        ...
        <div class="card card-primary card-outline">
              ...
              <div class="col-xl-2 col-lg-3 col-md-3 col-sm-4 mobile_cls">
                <div class="form-group">
                  <select name="store_id" id="store_id" class="form-control select2"  data-placeholder="Select Store" data-dropdown-css-class="select2-primary">
...
<div class="modal fade" id="add_modal">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <form method="POST" action="{{ url('api/operator') }}" id="add_staff">
          @csrf
...
<div class="modal fade" id="notificationModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{ url('operator/send_notification') }}" id="sendUserNotification">
          @csrf
...
$(document).ready(function(){
  ...
  $(document).on('click','.sendUserNotification',function(e){
      ...
      var _this=$(this); 
        ...
        if (title && message) {
          $.ajax({
            url:'{{ url('operator/send_notification') }}',
            dataType:'json',
            data:values,
            type:'POST',
...
function getWalletAmout() {
  ...
  $.ajax({
      url:'{{ url('operator/getWalletData') }}',
      dataType:'json',
      data:values,
      type:'POST',
...
ajax_datatable = $('#staff_listing').DataTable({
    ...
    serverSide: true,
     ...
     ajax:{
          url:'{{ url('api/operator') }}',
          data: function (d) {
              d.from_date = $('input[name=from_date]').val();
              d.to_date = $('input[name=to_date]').val();
              d.restaurant_id = $('select[name=store_id]').val();
              //alert(d.country);
          }
        },
    ...
    rowCallback: function(row, data, iDisplayIndex) {
      ...
      @can('Operator-edit')
        ...
        links += `<a href="#" data-staff_id="${data.id}" title="Send Notification" class="btn btn-secondary btn-xs send_notification" ><span class="fa fa-bell"></span></a>`;
...
$(document).on('click','.change_status',function(e){
      ...
      if(status == 'active'){
        var response = confirm('Are you sure want to active this operator account?');
      }else{
        var response = confirm('Are you sure want to deactive this operator account?');
      }
      if(response){
        ...
        $.ajax({
          ...
          dataType:'json',
          url: "{!! url('operator/changeStatus' )!!}" + "/" + id +'/'+status,
          success:function(res){
            if(res.status === 1){
              toastr.success(res.message);
              ajax_datatable.draw();
    ...
    $(document).on('click','.delete_staff',function(e){
          e.preventDefault();
          var response = confirm('Are you sure want to delete this operator?');
          if(response){
            id = $(this).data('staff_id');
            $.ajax({
              type: 'post',
              ...
              dataType:'json',
              url: "{!! url('api/operator' )!!}" + "/" + id,
              success:function(){
                toastr.success('{{ __('Operator is deleted successfully') }}');
                ajax_datatable.draw();
              },
              error:function(jqXHR,textStatus,textStatus){
                console.log(jqXHR);
