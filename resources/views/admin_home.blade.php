...
<div class="row page-titles">
  ...
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">{{ __('backend.home') }}</a></li>
...
<div class="row dashboard-page">
  ...
  <div class="col-md-4 col-sm-6">
    <div class="card bg-light">
      <div class="card-body">
        <a class="card_bd" href="{{ route('admin.players') }}">
          <div class="round bg-info">
            <!-- <img src="{{ URL::asset('assets/images/court.svg')}}"> -->
            <i class="fas fa-users" style="font-size: 20px;"></i>
          </div>
          <div class="card_bd_s">
            <h2 class="m-t-10 m-b-0">{{$no_of_players}}</h2>
            <small>{{ __('backend.Registered_Players') }}</small>
          </div>
        </a>
      </div>
    </div>
  ...
  <div class="col-md-4 col-sm-6">
    <div class="card bg-light">
      <div class="card-body">
        <a class="card_bd" href="{{ route('admin.facilities') }}">
          <div class="round bg-info">
            <!-- <img src="{{ URL::asset('assets/images/booking.svg')}}"> -->
            <i class="fab fa-servicestack" style="font-size: 20px;"></i>
          </div>
          <div class="card_bd_s">
            ...
            <small>{{ __('backend.Facilities') }}</small>
          </div>
        </a>
      </div>
    </div>
  ...
  <div class="col-md-4 col-sm-6">
    <div class="card bg-light">
      <div class="card-body">
        <a class="card_bd" href="{{ route('admin.facility_owner') }}">
          <div class="round bg-info">
            <!-- <img src="{{ URL::asset('assets/images/booking.svg')}}"> -->
          ...
          <div class="card_bd_s">
            <h2 class="m-t-10 m-b-0">
              <!-- <sup class="font-16">$</sup> -->
              {{$no_of_facility_owners}}
            </h2>
            <small>{{ __('backend.Facility_Owners') }}</small>
          </div>
        </a>
      </div>
    </div>
        ...
        <div class="tabbable-line">
          <div class="table-responsive">
            <table class="table table-striped table-hover table-bordered">
              ...
              <tbody>
                <tr>
                  @foreach($get_most_popular_booking as $key => $data)
                <tr>
        ...
        <div class="tabbable-line">
          <div class="table-responsive">
            <table class="table table-striped table-hover table-bordered">
              ...
              <tbody>
                @foreach($get_most_booking_users as $key => $user)
                <tr>
        ...
        <div class="tabbable-line">
          <div class="table-responsive">
            <table class="table table-striped table-hover table-bordered">
              ...
              <tbody>
                @foreach($get_recent_orders as $key => $order)
                <tr>
        ...
        <div class="tabbable-line">
          <div class="table-responsive">
            <table class="table table-striped table-hover table-bordered">
              ...
              <tbody>
                @foreach($get_cash_booking as $key => $order)
                <tr>
                  <td>{{ $order->id ?? 'No Order Id'}}</td>
                  <td>{{ $order->user_name ? $order->user_name:'No User Name'}}</td>
                  <td>{{ $order->courtDetails->court_name ?? 'No Court Name'}}</td>
                  ...
                  <div class="dropdown dropdown{{$order->id}}">
                    ...
                    <!-- <div class="dropdown">
                      <select class="form-control change_status" id="change_status" onchange="change_status(this)" data-id="{{$order->id}}">
                        <option value="Accepted" {{ $order->order_status=='Accepted'?'selected':'' }}>{{ __('backend.Accepted') }}</option>
                        <option value="Cancelled" {{ $order->order_status=='Cancelled'?'selected':'' }}>{{ __('backend.Cancelled') }}</option>
                        <option value="Pending" {{ $order->order_status=='Pending'?'selected':'' }}>{{ __('backend.Pending') }}</option>
                      </select>
                    </div> -->
                  </td>
                  <td>{{date('d-m-Y', strtotime($order->created_at))}}</td>
