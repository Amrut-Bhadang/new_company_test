...
<tr class="attributes_values_tr_{{$category_id.$count}}">
	<?php foreach ($records as $k => $v) { ?>
		<td>
			...
			<select name="attribute[{{$count}}][attribute_value_lang_id][{{$v->id}}]" id="attribute_value_id" class="form-control select2" data-placeholder="Select Attribute" style="width: 100%;">
