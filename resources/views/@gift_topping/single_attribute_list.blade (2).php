...
<div class="form-group">
	<div class="table-responsive">
		<table id="attribute_listing" class="table table-striped table-bordered" style="width:100%">
			<?php foreach ($records as $k => $v) { if ($v->attribute_values && $v->attribute_values->isNotEmpty()) { ?>
				<thead>
					<tr>
						<th>{{$v->name}}</th>
				...
				<tbody>
					<tr class="attributes_values_tr attributes_values_tr_new_{{$v->id}} attributes_values_tr_{{$k}}">
						<td>
							...
							<select name="attribute[{{$v->id.$k}}][attribute_value_lang_id][{{$v->id}}]" id="attribute_value_id" class="form-control select2" data-placeholder="Select Attribute" style="width: 100%;">
						...
						<td>
							<select name="attribute[{{$v->id.$k}}][is_mandatory]" class="form-control is_mandatory is_mandatory_{{$v->id}}" disabled style="width: 100%;">
						...
						<!-- <td>
							<select name="attribute[{{$v->id.$k}}][is_free]" onchange="disabledPrice(<?php echo $v->id.'0' ?>)" disabled class="form-control is_free isFree_{{$v->id}} is_free_<?php echo $v->id.'0' ?>" style="width: 100%;">
								<option value=''>---Select---</option>
								<option value="1">Yes</option>
								<option selected value="0">No</option>
							</select>
