...
<div class="row">
  <div class="container-fluid">
    <form method="POST" action="{{ url('admin/courts/popular_timing/update/'.$popular_booking->court_id.'/'.$popular_booking->id) }}" enctype="" id="edit_popular_time">
      @csrf
                ...
                <div class="col-md-6">
                  <div class="position-relative form-group">
                    ...
                    <select name="day" id="day" data-parsley-required="true" class="form-control select2" data-placeholder="{{__('backend.Popular_Day')}}" data-dropdown-css-class="select2-primary">
                      ...
                      <option value="Sunday" {{'Sunday' == $popular_booking->day ? 'selected':''}}>{{__('backend.Sunday')}}</option>
                      <option value="Monday" {{'Monday' == $popular_booking->day ? 'selected':''}}>{{__('backend.Monday')}}</option>
                      <option value="Tuesday" {{'Tuesday' == $popular_booking->day ? 'selected':''}}>{{__('backend.Tuesday')}}</option>
                      <option value="Wednesday" {{'Wednesday' == $popular_booking->day ? 'selected':''}}>{{__('backend.Wednesday')}}</option>
                      <option value="Thursday" {{'Thursday' == $popular_booking->day ? 'selected':''}}>{{__('backend.Thursday')}}</option>
                      <option value="Friday" {{'Friday' == $popular_booking->day ? 'selected':''}}>{{__('backend.Friday')}}</option>
                      <option value="Saturday" {{'Saturday' == $popular_booking->day ? 'selected':''}}>{{__('backend.Saturday')}}</option>
                    </select>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="position-relative form-group">
                    ...
                    <select name="time" id="time" data-parsley-required="true" class="form-control select2" data-placeholder="{{__('backend.Start_Time')}}" data-dropdown-css-class="select2-primary">
                      ...
                      <option value="{{$value['start_time']}}"{{$value['start_time'] == $popular_booking->time ? 'selected':''}}>{{$value['start_time'].' -To- '. $value['end_time']}}</option>
                      <?php } ?>
                    </select>
                  </div>
                </div>
...
<script>
  $('.select2').select2();
  $('#edit_popular_time').parsley();
    $(document).on('submit', "#edit_popular_time",function(e){
      e.preventDefault();
      var _this=$(this);
        $('#group_loader').fadeIn();
      ...
      formData.append('_method', 'POST');
        $.ajax({
        url:"{{ url('admin/courts/popular_timing/update/'.$popular_booking->court_id.'/'.$popular_booking->id) }}",
        dataType:'json',
        data:formData,
        type:'POST',
        cache:false,
        ...
        success:function(res){
              if(res.status === 1){
               window.location.href = "{{url('admin/courts/popular_timing/'.$popular_booking->court_id)}}";
                toastr.success(res.message);
                $('#edit_popular_time')[0].reset();
                $('#edit_popular_time').parsley().reset();
              //console.log('helo');
              // window.location.href =  url('admin/courts/popular_timing'.$popular_booking->court_id);
                ajax_datatable.draw();
              }else{
