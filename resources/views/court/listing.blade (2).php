...
<div class="row page-titles">
  <div class="col-md-5 align-self-center">
    <h4 class="text-themecolor">{{ __('backend.Court_Manager') }}</h4>
  </div>
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/') }}">{{ __('backend.home') }}</a></li>
        <li class="breadcrumb-item active">{{ __('backend.Court_Manager') }}</li>
      </ol>
      @can('Court-create')
      <a href="{{ url('admin/courts/create') }}" class="btn btn-info d-none d-lg-block m-l-15" title="{{ __('Add Court') }}"><i class="fa fa-plus"></i> {{ __('backend.Add_Court') }}</a>
        ...
        <div class="card-body">
          <form method="POST" id="search-form" class="form-inline-sec" role="form">
            <div class="row">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                <div class="form-group">
                  <select name="facility_id" id="facility_id" class="form-control select2" data-placeholder="{{ __('backend.Select_facility') }}" data-dropdown-css-class="select2-primary">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                <div class="form-group">
                  <select name="court_status" id="court_status" class="form-control select2" data-placeholder="{{ __('backend.Select_court_status') }}" data-dropdown-css-class="select2-primary">
                    <option value="">--{{ __('backend.Select_court_status') }}--</option>
            ...
            <table id="staff_listing" class="table table-striped table-bordered" style="width:100%">
              <thead>
                <tr>
                  ...
                  <th>{{ __('backend.Facility') }}</th>
                  <th>{{ __('backend.Category') }}</th>
                  <th>{{ __('backend.Court_Name') }}</th>
                  <th>{{ __('backend.Address') }}</th>
                  <th>{{ __('backend.Minimum_Hours_Book') }} ( {{__('backend.Time_Slot')}} )</th>
                  <th>{{ __('backend.Hourly_Price') }}</th>
                  <th>{{ __('backend.Admin_commission') }}</th>
                  <th>{{ __('backend.Position') }}</th>
                  <th>{{ __('backend.Is_Featured') }}</th>
...
<script>
    ...
    ajax_datatable = $('#staff_listing').DataTable({
      ...
      ajax: {
        ...
        data: function(d) {
          ...
          d.court_status = $('select[name=court_status]').val();
          d.facility_id = $('select[name=facility_id]').val();
          d.category_id = $('select[name=category_id]').val();
          d.url_facilityId = "{{$facilityId}}";
      ...
      columns: [
        ...
        {
          data: 'court_category_name',
          name: 'court_categories.name'
        },
        {
          data: 'court_name',
          name: 'court_name'
      ...
      rowCallback: function(row, data, iDisplayIndex) {
        ...
        @can('Court-edit')
        links += `<a href="{{ url('admin/courts/edit') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Edit_Details') }}" class="btn btn-info btn-xs" ><span class="fa fa-edit"></span></a>`;
        @endcan
        @can('Court-edit')
        if (user_type == 0) {
          ...
          links += `<a href="{{ url('admin/courts/manage_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Time_Slot') }}" class="btn btn-warning btn-xs" ><span class="fa fa-clock"></span></a>`;
          links += `<a href="{{ url('admin/courts/popular_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Popular_Time') }}" class="btn btn-warning btn-xs" ><span class="fas fa-sun"></span></a>`;
        } else {
          links += `<a href="#" data-staff_id="${data.id}" title="{{ __('backend.View_Details') }}" class="btn btn-primary btn-xs view_btn" ><span class="fa fa-eye"></span></a>`;
          links += `<a href="#" data-staff_id="${data.id}" title="{{ __('backend.Delete') }}" data-is_featured="delete_yes" class="change_is_delete"><span class='btn btn-warning btn-xs'><span class="fa fa-trash"></span></span></a>`;
          links += `<a href="{{ url('admin/courts/popular_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Popular_Time') }}" class="btn btn-warning btn-xs" ><span class="fas fa-sun"></span></a>`;
         
          // links += `<a href="{{ url('admin/courts/manage_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.timing') }}" class="btn btn-warning btn-xs" ><span class="fa fa-clock"></span></a>`;
        }
        @endcan
        /*@can('Court-edit')
        links += `<a href="{{ url('admin/courts/view_address').'/' }}${data.id}" data-staff_id="${data.id}" title="View Address" class="btn btn-danger btn-xs view_address" ><span class="fa fa-home"></span></a>`;
        @endcan
        @can('Court-edit')
        links += `<a href="{{ url('admin/courts/transaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Transaction" class="btn btn-dark btn-xs view_transaction" ><span class="fa fa-credit-card"></span></a>`;
        @endcan
        @can('Court-edit')
        // links += `<a href="{{ url('admin/courts/giftstransaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Gifts Transaction" class="btn btn-secondary btn-xs view_transaction" ><span class="fa fa-gifts"></span></a>`;
        ...
        } else if(data.admin_commission == 0){
          admin_commission += `<span> ${data.admin_commission} % </span>`;
        }else {
  ...
  $(document).on('click', '.change_status', function(e) {
    ...
    if (status == 'active') {
      var response = confirm("{{ __('backend.confirm_box_active') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_deactive') }}");
    }
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/courts/changeStatus' )!!}" + "/" + id + '/' + status,
  ...
  $(document).on('click', '.change_is_featured', function(e) {
    ...
    if (is_featured == 'active') {
      var response = confirm("{{ __('backend.confirm_box_is_featured_yes_court') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_is_featured_no_court') }}");
    }
    if (response) {
      id = $(this).data('staff_id');
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/courts/changeIsFeatured' )!!}" + "/" + id + '/' + is_featured,
        success: function(res) {
          if (res.status === 1) {
            toastr.success(res.message);
            ajax_datatable.draw();
  ...
  $(document).on('click', '.change_is_delete', function(e) {
    ...
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/courts/delete_court' )!!}" + "/" + id + '/' + is_delete,
        success: function(res) {
          if (res.status === 1) {
            toastr.success(res.message);
            ajax_datatable.draw();
