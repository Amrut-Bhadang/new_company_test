...
<div class="row page-titles">
  <div class="col-md-5 align-self-center">
    <h4 class="text-themecolor">{{ __('backend.Facility_Manager') }}</h4>
  </div>
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/') }}">{{ __('backend.home') }}</a></li>
        <li class="breadcrumb-item active">{{ __('backend.Facility_Manager') }}</li>
      </ol>
      @can('Facility-create')
      <a href="{{ url('admin/facilities/create') }}" class="btn btn-info d-none d-lg-block m-l-15" title="{{ __('backend.Add_facility') }}"><i class="fa fa-plus"></i> {{ __('backend.Add_facility') }}</a>
      <a href="{{ url('admin/facilities/export/0') }}" class="btn btn-warning d-none d-lg-block m-l-15" title="{{ __('backend.Export') }}"><i class="fa fa-download"></i> {{ __('backend.Export') }}</a>
      @endcan
    </div>
  </div>
</div>
          ...
          <form method="POST" id="search-form" class="form-inline-sec" role="form">
            <div class="row">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                <div class="form-group">
                  <select name="facility_status" id="facility_status" class="form-control select2" data-placeholder="{{ __('backend.Select_facility_status') }}" data-dropdown-css-class="select2-primary">
                    <option value="">--{{ __('backend.Select_facility_status') }}--</option>
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                <div class="form-group">
                  <select name="facility_owner_id" id="facility_owner_id" class="form-control select2" data-placeholder="{{ __('backend.Select_facility_owner') }}" data-dropdown-css-class="select2-primary">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                <div class="form-group">
                  <select name="amenities_id" id="amenities_id" class="form-control select2" data-placeholder="{{ __('backend.Select_amenities') }}" data-dropdown-css-class="select2-primary">
                    <option value="">--{{ __('backend.Select_amenities') }}--</option>
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                <div class="form-group">
                  <select name="category_id" id="category_id" class="form-control select2" data-placeholder="{{ __('backend.Select_Category') }}" data-dropdown-css-class="select2-primary">
          ...
          <div class="table-responsive">
            <table id="staff_listing" class="table table-striped table-bordered" style="width:100%">
              <thead>
                <tr>
                  <!-- <th>{{ __('Sr. no') }}</th> -->
                  <th>{{ __('backend.Facility_image') }}</th>
                  @php $login_user = Auth::user(); @endphp
                  @if($login_user->type == '0')
                  <th>{{ __('backend.Facility_Owner') }}</th>
                  @endif
                  <th>{{ __('backend.Facility_Name') }}</th>
                  <th>{{ __('backend.Address') }}</th>
                  <th>{{ __('backend.Amenities') }}</th>
...
<div class="modal fade" id="add_modal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form method="POST" action="{{ url('admin/api/facilities') }}" id="add_staff">
        @csrf
...
<div class="modal fade" id="notificationModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{ url('admin/facilities/send_notification') }}" id="sendUserNotification">
          @csrf
...
<div class="modal fade" id="viewModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">{{ __('backend.View_facility') }}</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
      ...
      ajax_datatable = $('#staff_listing').DataTable({
        ...
        ajax: {
          ...
          data: function(d) {
            ...
            d.facility_status = $('select[name=facility_status]').val();
            d.facility_owner_id = $('select[name=facility_owner_id]').val();
            d.amenities_id  = $('select[name=amenities_id]').val();
        ...
        columns: [
          ...
          {
            data: 'facility_amenities',
            name: 'facility_amenities',
            orderable: false,
            searchable: false
          },
          {
            data: 'facility_categories',
        ...
        rowCallback: function(row, data, iDisplayIndex) {
          ...
          @can('Facility-edit')
          links += `<a href="{{ url('admin/facilities/edit') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Edit_Details') }}" class="btn btn-info btn-xs" ><span class="fa fa-edit"></span></a>`;
          links += `<a href="{{ url('admin/courts') }}?facility=${data.id}"  title="{{ __('backend.View_Courts') }}" class="btn btn-info btn-xs" ><span class="fas fa-ring"></span></a>`;
          ...
          // links += `<a href="{{ url('admin/facilities/manage_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.timing') }}" class="btn btn-warning btn-xs" ><span class="fa fa-clock"></span></a>`;
          @endcan
          /*@can('Facility-edit')
          links += `<a href="{{ url('admin/facilities/view_address').'/' }}${data.id}" data-staff_id="${data.id}" title="View Address" class="btn btn-danger btn-xs view_address" ><span class="fa fa-home"></span></a>`;
          @endcan
          @can('Facility-edit')
          links += `<a href="{{ url('admin/facilities/transaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Transaction" class="btn btn-dark btn-xs view_transaction" ><span class="fa fa-credit-card"></span></a>`;
          @endcan
          @can('Facility-edit')
          // links += `<a href="{{ url('admin/facilities/giftstransaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Gifts Transaction" class="btn btn-secondary btn-xs view_transaction" ><span class="fa fa-gifts"></span></a>`;
          ...
          position += `{{__('backend.No_Position')}}`;
        } else {
          position += `<span> ${data.position}</span>`;
        }

         if (data.commission == '' || data.commission == null) {
          commission += `{{__('backend.No_Data')}}`;
        } else {
          commission += `<span> ${data.commission}</span>`;
        }

          links += `</div>`;
          total_order += `<a href="{{ url('admin/facilities/view_orders').'/' }}${data.id}" data-staff_id="${data.id}" class="view_order"><span class='label label-rounded label-primary'>${data.total_order}  Orders</span></a>`;

          $('td:eq(7)', row).html(commission);
          $('td:eq(8)', row).html(status);
          $('td:eq(0)', row).html(image);
          $('td:eq(10)', row).html(links);
          $('td:eq(6)', row).html(position);
        },
      });
  ...
  $(document).on('click', '.change_is_delete', function(e) {
    ...
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/facilities/delete_facilities' )!!}" + "/" + id + '/' + is_delete,
  ...
  $(document).on('click', '.change_status', function(e) {
    ...
    if (status == 'active') {
      var response = confirm("{{ __('backend.confirm_box_active_facility') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_deactive_facility') }}");
    }
    if (response) {
      id = $(this).data('staff_id');
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/facilities/changeStatus' )!!}" + "/" + id + '/' + status,
        success: function(res) {
          if (res.status === 1) {
            toastr.success(res.message);
            ajax_datatable.draw();
  ...
  $(document).on('click', '.sendUserNotification', function(e) {
    ...
    if (title && message) {
      $.ajax({
        url: '{{ url("admin/facilities / send_notification ") }}',
        dataType: 'json',
        data: values,
        type: 'POST',
        beforeSend: function() {
  ...
  $(document).on('click', '.view_btn', function(e) {
    ...
    $.ajax({
      url: '{{url("admin/facilities/view")}}/' + id,
      dataType: 'html',
      success: function(result) {
  ...
  $(document).on('click', '.delete_staff', function(e) {
    ...
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/api/facilities' )!!}" + "/" + id,
        success: function() {
