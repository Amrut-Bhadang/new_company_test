...
<div class="row page-titles">
  <div class="col-md-5 align-self-center">
    <h4 class="text-themecolor">{{ __('backend.User_bank_detail_Manager') }}</h4>
  </div>
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/') }}">{{ __('backend.home') }}</a></li>
        <li class="breadcrumb-item active">{{ __('backend.User_bank_detail_Manager') }}</li>
      </ol>
      @can('User_bank_detail-create')
      <a href="{{ url('admin/user_bank_detail/create') }}" class="btn btn-info d-none d-lg-block m-l-15" title="{{ __('backend.Add_Bank_Detail') }}"><i class="fa fa-plus"></i> {{ __('backend.Add_Bank_Detail') }}</a>
      <!-- <a href="{{ url('admin/user_bank_detail/export/0') }}" class="btn btn-warning d-none d-lg-block m-l-15" title="{{ __('backend.Export') }}"><i class="fa fa-download"></i> {{ __('backend.Export') }}</a> -->
      @endcan
    </div>
  </div>
</div>
          ...
          <form method="POST" id="search-form" class="form-inline-sec" role="form">
            <div class="row">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                  <div class="form-group">
                    <select name="bank_detail_status" id="bank_detail_status" class="form-control select2" data-placeholder="{{ __('backend.Select_bank_detail_status') }}" data-dropdown-css-class="select2-primary">
                        <option value="">--{{ __('backend.Select_bank_detail_status') }}--</option>
          ...
          <div class="table-responsive">
            <table id="staff_listing" class="table table-striped table-bordered" style="width:100%">
              <thead>
                <tr>
                  <!-- <th>{{ __('Sr. no') }}</th> -->
                  <th>{{ __('backend.Passbook_Image') }}</th>
                  <th>{{ __('backend.Account_Holder_Name') }}</th>
                  <th>{{ __('backend.Bank_Name') }}</th>
                  <th>{{ __('backend.Bank_Code') }}</th>
                  <th>{{ __('backend.Account_Type') }}</th>
                  <th>{{ __('backend.Account_Number') }}</th>
...
<div class="modal fade" id="add_modal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form method="POST" action="{{ url('admin/api/user_bank_detail') }}" id="add_staff">
        @csrf
...
<div class="modal fade" id="notificationModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{ url('admin/user_bank_detail/send_notification') }}" id="sendUserNotification">
          @csrf
...
<div class="modal fade" id="viewModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">{{ __('backend.View_Bank_Detail') }}</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
    ...
    ajax_datatable = $('#staff_listing').DataTable({
      ...
      ajax: {
        ...
        data: function(d) {
          ...
          d.bank_detail_status = $('select[name=bank_detail_status]').val();
      ...
      rowCallback: function(row, data, iDisplayIndex) {
        ...
        @can('User_bank_detail-edit')
        links += `<a href="{{ url('admin/user_bank_detail/edit') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Edit_Details') }}" class="btn btn-info btn-xs" ><span class="fa fa-edit"></span></a>`;
        @endcan
        // @can('User_bank_detail-delete')
        ...
        // links += `<a href="{{ url('admin/user_bank_detail/manage_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.timing') }}" class="btn btn-warning btn-xs" ><span class="fa fa-clock"></span></a>`;
        @endcan
        /*@can('User_bank_detail-edit')
        links += `<a href="{{ url('admin/user_bank_detail/view_address').'/' }}${data.id}" data-staff_id="${data.id}" title="View Address" class="btn btn-danger btn-xs view_address" ><span class="fa fa-home"></span></a>`;
        @endcan
        @can('User_bank_detail-edit')
        links += `<a href="{{ url('admin/user_bank_detail/transaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Transaction" class="btn btn-dark btn-xs view_transaction" ><span class="fa fa-credit-card"></span></a>`;
        @endcan
        @can('User_bank_detail-edit')
        // links += `<a href="{{ url('admin/user_bank_detail/giftstransaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Gifts Transaction" class="btn btn-secondary btn-xs view_transaction" ><span class="fa fa-gifts"></span></a>`;
        ...
        if (data.passbook_image) {
          passbook_image = `<img width="100" height="100" src="${data.passbook_image}"/>`;
        }
        ...
        links += `</div>`;
        total_order += `<a href="{{ url('admin/user_bank_detail/view_orders').'/' }}${data.id}" data-staff_id="${data.id}" class="view_order"><span class='label label-rounded label-primary'>${data.total_order}  Orders</span></a>`;

        // $('td:eq(2)', row).html(email);
        // $('td:eq(3)', row).html(mobile);
        $('td:eq(6)', row).html(status);
        $('td:eq(0)', row).html(passbook_image);
  ...
  $(document).on('click', '.change_status', function(e) {
    ...
    if (status == 'active') {
      var response = confirm("{{ __('backend.confirm_box_active_bank_detail') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_deactive_bank_detail') }}");
    }
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/user_bank_detail/changeStatus' )!!}" + "/" + id + '/' + status,
  ...
  $(document).on('click', '.sendUserNotification', function(e) {
    ...
    if (title && message) {
      $.ajax({
        url: '{{ url("admin/user_bank_detail / send_notification ") }}',
        dataType: 'json',
        data: values,
        type: 'POST',
        beforeSend: function() {
  ...
  $(document).on('click', '.view_btn', function(e) {
    ...
    $.ajax({
      url: '{{url("admin/user_bank_detail/view")}}/' + id,
      dataType: 'html',
      success: function(result) {
  ...
  $(document).on('click', '.delete_staff', function(e) {
    ...
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/api/user_bank_detail' )!!}" + "/" + id,
        success: function() {
