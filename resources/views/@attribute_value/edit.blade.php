...
<!-- <div class="row">
    <div class="col-md-6" style="margin-bottom: 10px;">
       <a href="{{ url('attribute_value') }}" class="btn btn-primary btn-sm" ><i class="fa fa-arrow-left"></i> Back </a>
    </div>
</div> -->
<div class="content">
  <div class="row">
        <form method="POST" action="{{ url('api/attribute_value'.$attribute_value->id) }}" id="edit_attribute" style="width: 100%">
            @csrf   
            <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select disabled="" name="main_category_id" id="main_category_id" class="form-control select3" data-placeholder="Select Service" style="width: 100%;" data-parsley-required="true" >
                                            ...
                                            @foreach ($main_category as $main_category)
                                                <option value="{{ $main_category->id }}" {{ $attribute_value->main_category_id == $main_category->id?'selected':'' }} >{{ $main_category->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                      ...
                                      @foreach ($attribute_value_langs as $key => $attribute)
                                        <div id="TextBoxDivEdit{{$key+1}}">
                                          ...
                                          if (isset($attribute->id)) {
                                              $langData_en = AttributeValueLang::where(['id'=>$attribute->id])->first();  
                                          }
                                          ?>
                                            <br/>
...
<script type="text/javascript">
    ...
    function getCategory() {
        ...
        var category_id = "<?php echo $attribute_value->category_id ?>";
        $.ajax({
           url:'{{url('attribute_value/show_category')}}/'+main_category_id+'/'+category_id,
           dataType: 'html',
...
$("#edit_attribute").on('submit',function(e){ 
  e.preventDefault();
    ...
    $.ajax({
    url:'{{ url('api/attribute_value/'.$attribute_value->id) }}',
    dataType:'json',
    data:formData,
    type:'POST',
    cache:false,
