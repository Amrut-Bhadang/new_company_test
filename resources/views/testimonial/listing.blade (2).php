...
<div class="row page-titles">
  <div class="col-md-5 align-self-center">
    <h4 class="text-themecolor">{{ __('backend.Testimonial_Manager') }}</h4>
  </div>
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/') }}">{{ __('backend.home') }}</a></li>
        <li class="breadcrumb-item active">{{ __('backend.Testimonial_Manager') }}</li>
      </ol>
      @can('Testimonial-create')
      <a href="{{ url('admin/testimonial/create') }}" class="btn btn-info d-none d-lg-block m-l-15" title="{{ __('backend.Add_testimonial') }}"><i class="fa fa-plus"></i> {{ __('backend.Add_testimonial') }}</a>
          ...
          <form method="POST" id="search-form" class="form-inline-sec" role="form">
            <div class="row">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                  <div class="form-group">
                    <select name="testimonial_status" id="testimonial_status" class="form-control select2" data-placeholder="{{ __('backend.Select_testimonial_status') }}" data-dropdown-css-class="select2-primary">
                        <option value="">--{{ __('backend.Select_testimonial_status') }}--</option>
              ...
              <!-- <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                  <div class="form-group">
                    <select name="testimonial_owner_name" id="testimonial_owner_name" class="form-control select2" data-placeholder="{{ __('backend.Select_testimonial_owner') }}" data-dropdown-css-class="select2-primary">
                        <option value="">--{{ __('backend.Select_testimonial_owner') }}--</option>
              ...
              <!-- <div class="col-xl-2 col-lg-3 col-md-3 col-sm-4 mobile_cls">
                <div class="form-group">
                  <select name="testimonial_id" id="testimonial_id" class="form-control select2"  data-placeholder="Select Testimonial" data-dropdown-css-class="select2-primary">
...
<div class="modal fade" id="add_modal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form method="POST" action="{{ url('admin/api/testimonial') }}" id="add_staff">
        @csrf
        ...
        <div class="modal-body">
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label class="control-label" for="testimonial_id">Testimonial</label>
                <select name="testimonial_id" id="testimonial_id" class="form-control select2" data-placeholder="Select Testimonial" data-dropdown-css-class="select2-primary">
                  <option value="">--Select Testimonial--</option>
...
<div class="modal fade" id="notificationModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{ url('admin/testimonial/send_notification') }}" id="sendUserNotification">
          @csrf
...
<div class="modal fade" id="viewModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">{{ __('backend.View_testimonial') }}</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
    ...
    ajax_datatable = $('#staff_listing').DataTable({
      ...
      ajax: {
        ...
        data: function(d) {
          ...
          d.testimonial_status = $('select[name=testimonial_status]').val();
          d.testimonial_owner_name = $('select[name=testimonial_owner_name]').val();
      ...
      rowCallback: function(row, data, iDisplayIndex) {
        ...
        links += `<a href="{{ url('admin/testimonial/edit') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Edit_Details') }}" class="btn btn-info btn-xs" ><span class="fa fa-edit"></span></a>`;
        @endcan
        ...
        // links += `<a href="{{ url('admin/testimonial/manage_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.timing') }}" class="btn btn-warning btn-xs" ><span class="fa fa-clock"></span></a>`;
        @endcan
        /*@can('Testimonial-edit')
        links += `<a href="{{ url('admin/testimonial/view_address').'/' }}${data.id}" data-staff_id="${data.id}" title="View Address" class="btn btn-danger btn-xs view_address" ><span class="fa fa-home"></span></a>`;
        @endcan
        @can('Testimonial-edit')
        links += `<a href="{{ url('admin/testimonial/transaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Transaction" class="btn btn-dark btn-xs view_transaction" ><span class="fa fa-credit-card"></span></a>`;
        @endcan
        @can('Testimonial-edit')
        // links += `<a href="{{ url('admin/testimonial/giftstransaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Gifts Transaction" class="btn btn-secondary btn-xs view_transaction" ><span class="fa fa-gifts"></span></a>`;
        ...
        links += `</div>`;
        total_order += `<a href="{{ url('admin/testimonial/view_orders').'/' }}${data.id}" data-staff_id="${data.id}" class="view_order"><span class='label label-rounded label-primary'>${data.total_order}  Orders</span></a>`;
  ...
  $(document).on('click', '.change_status', function(e) {
    ...
    if (status == 'active') {
      var response = confirm("{{ __('backend.confirm_box_active_testimonial') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_deactive_testimonial') }}");
    }
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/testimonial/changeStatus' )!!}" + "/" + id + '/' + status,
  ...
  $(document).on('click', '.sendUserNotification', function(e) {
    ...
    if (title && message) {
      $.ajax({
        url: '{{ url("admin/testimonial / send_notification ") }}',
        dataType: 'json',
        data: values,
        type: 'POST',
        beforeSend: function() {
  ...
  $(document).on('click', '.view_btn', function(e) {
    ...
    $.ajax({
      url: '{{url("admin/testimonial/view")}}/' + id,
      dataType: 'html',
      success: function(result) {
  ...
  $(document).on('click', '.delete_staff', function(e) {
    e.preventDefault();
    var response = confirm("{{ __('backend.confirm_box_delete_testimonial') }}");
    if (response) {
      id = $(this).data('staff_id');
      $.ajax({
        type: 'post',
        ...
        dataType: 'json',
        url: "{!! url('admin/api/testimonial' )!!}" + "/" + id,
        success: function() {
          toastr.success("{{ __('backend.testimonial_deleted_successfully') }}");
          ajax_datatable.draw();
        },
        error: function(jqXHR, textStatus, textStatus) {
          console.log(jqXHR);
