...
<div class="row page-titles">
  <div class="col-md-5 align-self-center">
    <h4 class="text-themecolor">{{ __('backend.Player_Manager') }}</h4>
  </div>
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/') }}">{{ __('backend.home') }}</a></li>
        <li class="breadcrumb-item active">{{ __('backend.Player_Manager') }}</li>
      </ol>
      @can('Player-create')
      <a href="{{ url('admin/players/create') }}" class="btn btn-info d-none d-lg-block m-l-15" title="{{ __('backend.Add_player') }}"><i class="fa fa-plus"></i> {{ __('backend.Add_player') }}</a>
      <a href="{{ url('admin/players/export/0') }}" class="btn btn-warning d-none d-lg-block m-l-15" title="{{ __('backend.Export') }}"><i class="fa fa-download"></i> {{ __('backend.Export') }}</a>
      @endcan
    </div>
  </div>
</div>
          ...
          <form method="POST" id="search-form" class="form-inline-sec" role="form">
            <div class="row">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                  <div class="form-group">
                    <select name="player_status" id="player_status" class="form-control select2" data-placeholder="{{ __('backend.Select_player_status') }}" data-dropdown-css-class="select2-primary">
                        <option value="">--{{ __('backend.Select_player_status') }}--</option>
          ...
          <div class="table-responsive">
            <table id="staff_listing" class="table table-striped table-bordered" style="width:100%">
              <thead>
                <tr>
                  <!-- <th>{{ __('Sr. no') }}</th> -->
                  <th>{{ __('backend.Profile_Pic') }}</th>
                  <th>{{ __('backend.Name') }}</th>
                  <th>{{ __('backend.Email') }}</th>
                  <th>{{ __('backend.Mobile_Number') }}</th>
                  <th>{{ __('backend.Gender') }}</th>
                  <th>{{ __('backend.Status') }}</th>
                  <th>{{ __('backend.Is_Facility_Owner') }}</th>
                  <th>{{ __('backend.Registration_Date') }}</th>
...
<div class="modal fade" id="add_modal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form method="POST" action="{{ url('admin/api/players') }}" id="add_staff">
        @csrf
...
<div class="modal fade" id="notificationModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{ url('admin/players/send_notification') }}" id="sendUserNotification">
          @csrf
...
<div class="modal fade" id="viewModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">{{ __('backend.View_player') }}</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
    ...
    ajax_datatable = $('#staff_listing').DataTable({
      ...
      ajax: {
        ...
        data: function(d) {
          ...
          d.player_status = $('select[name=player_status]').val();
          //alert(d.country);
        }
      },
      columns: [
      ...
      rowCallback: function(row, data, iDisplayIndex) {
        ...
        var created_at = '';
        var image = '';
        var is_facility_owner = '';
        var is_deleted = '';
        var mobile = data.country_code+'-'+data.mobile;
        links += `<div class="btn-group" role="group" >`;
        @can('Player-edit')
        links += `<a href="{{ url('admin/players/edit') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Edit_Details') }}" class="btn btn-info btn-xs" ><span class="fa fa-edit"></span></a>`;
        @endcan
        // @can('Player-delete')
        ...
        // links += `<a href="{{ url('admin/players/manage_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.timing') }}" class="btn btn-warning btn-xs" ><span class="fa fa-clock"></span></a>`;
        @endcan
        /*@can('Player-edit')
        links += `<a href="{{ url('admin/players/view_address').'/' }}${data.id}" data-staff_id="${data.id}" title="View Address" class="btn btn-danger btn-xs view_address" ><span class="fa fa-home"></span></a>`;
        @endcan
        @can('Player-edit')
        links += `<a href="{{ url('admin/players/transaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Transaction" class="btn btn-dark btn-xs view_transaction" ><span class="fa fa-credit-card"></span></a>`;
        @endcan
        @can('Player-edit')
        // links += `<a href="{{ url('admin/players/giftstransaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Gifts Transaction" class="btn btn-secondary btn-xs view_transaction" ><span class="fa fa-gifts"></span></a>`;
        ...
        if (data.is_facility_owner == 1) {
          is_facility_owner += `<a href="#" data-staff_id="${data.id}" title="Change Is Facility Owner" data-status="no" class="change_is_facility_owner"><span class='label label-rounded label-success'>{{ __('backend.Yes') }}</span></a>`;
        } else {
          is_facility_owner += `<a href="#" data-staff_id="${data.id}" title="Change Is Facility_owner" data-status="yes" class="change_is_facility_owner"><span class='label label-rounded label-warning'>{{ __('backend.No') }}</span></a>`;
        }
        ...
        if (data.gender == null) {
         ...
         if (data.created_at == null) {
          ...
          created_at += `<span> ${data.created_at} </span>`;
        }

        

        links += `</div>`;
        total_order += `<a href="{{ url('admin/players/view_orders').'/' }}${data.id}" data-staff_id="${data.id}" class="view_order"><span class='label label-rounded label-primary'>${data.total_order}  Orders</span></a>`;

        $('td:eq(1)', row).html(name);
        $('td:eq(2)', row).html(email);
        $('td:eq(3)', row).html(mobile);
        $('td:eq(4)', row).html(gender);
        $('td:eq(5)', row).html(status);
        $('td:eq(6)', row).html(is_facility_owner);
        $('td:eq(0)', row).html(image);
        $('td:eq(7)', row).html(created_at);
  ...
  $(document).on('click', '.change_status', function(e) {
    ...
    if (status == 'active') {
      var response = confirm("{{ __('backend.confirm_box_active_player') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_deactive_player') }}");
    }
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/players/changeStatus' )!!}" + "/" + id + '/' + status,
  ...
  $(document).on('click', '.change_is_facility_owner', function(e) {
    e.preventDefault();
    status = $(this).data('status');
    if (status == 'yes') {
      var response = confirm("{{ __('backend.confirm_box_yes_is_facility_owner') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_no_is_facility_owner') }}");
    }
    if (response) {
      id = $(this).data('staff_id');
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/players/is_facility_owner' )!!}" + "/" + id + '/' + status,
        success: function(res) {
          if (res.status === 1) {
            toastr.success(res.message);
            ajax_datatable.draw();
  ...
  $(document).on('click', '.change_is_delete', function(e) {
    ...
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/players/delete_users' )!!}" + "/" + id + '/' + is_delete,
        success: function(res) {
          if (res.status === 1) {
            toastr.success(res.message);
            ajax_datatable.draw();
  ...
  $(document).on('click', '.sendUserNotification', function(e) {
    ...
    if (title && message) {
      $.ajax({
        url: '{{ url("admin/players / send_notification ") }}',
        dataType: 'json',
        data: values,
        type: 'POST',
        beforeSend: function() {
