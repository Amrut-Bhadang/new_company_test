...
?>
<form method="PUT" action="{{ url('api/gift_category/'.$category->id) }}" id="edit_role">
    @csrf
    ...
    <?php
      if(isset($category))
      {
          $langData = GiftCategoryLang::where(['lang'=>$lang,'gift_category_id'=>$category->id])->first();  
      } ?>
...
$("#edit_role").on('submit',function(e){ 
  ...
  var _this=$(this); 
    ...
    $.ajax({
      url:'{{ url('api/gift_category/'.$category->id) }}',
      dataType:'json',
      data:formData,
      type:'POST',
      cache:false,
