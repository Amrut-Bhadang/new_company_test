...
<div class="row page-titles">
  <div class="col-md-5 align-self-center">
    <h4 class="text-themecolor">{{ __('backend.Commission_Manager') }}</h4>
  </div>
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/') }}">{{ __('backend.home') }}</a></li>
        <li class="breadcrumb-item active">{{ __('backend.Commission_Manager') }}</li>
      </ol>
      @can('Commission-create')
      <a href="{{ url('admin/commission/create') }}" class="btn btn-info d-none d-lg-block m-l-15" title="{{ __('backend.Add_commission') }}"><i class="fa fa-plus"></i> {{ __('backend.Add_commission') }}</a>
          ...
          <form method="POST" id="search-form" class="form-inline-sec" role="form">
            <div class="row">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                  <div class="form-group">
                    <select name="commission_status" id="commission_status" class="form-control select2" data-placeholder="{{ __('backend.Select_commission_status') }}" data-dropdown-css-class="select2-primary">
                        <option value="">--{{ __('backend.Select_commission_status') }}--</option>
              ...
              <!-- <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                  <div class="form-group">
                    <select name="court_name" id="court_name" class="form-control select2" data-placeholder="{{ __('backend.Select_court') }}" data-dropdown-css-class="select2-primary">
              ...
              <!-- <div class="col-xl-2 col-lg-3 col-md-3 col-sm-4 mobile_cls">
                <div class="form-group">
                  <select name="commission_id" id="commission_id" class="form-control select2"  data-placeholder="Select Commission" data-dropdown-css-class="select2-primary">
...
<div class="modal fade" id="add_modal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form method="POST" action="{{ url('admin/api/commission') }}" id="add_staff">
        @csrf
        ...
        <div class="modal-body">
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label class="control-label" for="commission_id">Commission</label>
                <select name="commission_id" id="commission_id" class="form-control select2" data-placeholder="Select Commission" data-dropdown-css-class="select2-primary">
                  <option value="">--Select Commission--</option>
...
<div class="modal fade" id="notificationModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{ url('admin/commission/send_notification') }}" id="sendUserNotification">
          @csrf
...
<div class="modal fade" id="viewModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">{{ __('backend.View_commission') }}</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
    ...
    ajax_datatable = $('#staff_listing').DataTable({
      ...
      ajax: {
        ...
        data: function(d) {
          ...
          d.commission_status = $('select[name=commission_status]').val();
          d.court_name = $('select[name=court_name]').val();
      ...
      rowCallback: function(row, data, iDisplayIndex) {
        ...
        @can('Commission-edit')
        links += `<a href="{{ url('admin/commission/edit') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Edit_Details') }}" class="btn btn-info btn-xs" ><span class="fa fa-edit"></span></a>`;
        @endcan
        ...
        // links += `<a href="{{ url('admin/commission/manage_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.timing') }}" class="btn btn-warning btn-xs" ><span class="fa fa-clock"></span></a>`;
        @endcan
        /*@can('Commission-edit')
        links += `<a href="{{ url('admin/commission/view_address').'/' }}${data.id}" data-staff_id="${data.id}" title="View Address" class="btn btn-danger btn-xs view_address" ><span class="fa fa-home"></span></a>`;
        @endcan
        @can('Commission-edit')
        links += `<a href="{{ url('admin/commission/transaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Transaction" class="btn btn-dark btn-xs view_transaction" ><span class="fa fa-credit-card"></span></a>`;
        @endcan
        @can('Commission-edit')
        // links += `<a href="{{ url('admin/commission/giftstransaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Gifts Transaction" class="btn btn-secondary btn-xs view_transaction" ><span class="fa fa-gifts"></span></a>`;
        ...
        links += `</div>`;
        total_order += `<a href="{{ url('admin/commission/view_orders').'/' }}${data.id}" data-staff_id="${data.id}" class="view_order"><span class='label label-rounded label-primary'>${data.total_order}  Orders</span></a>`;
  ...
  $(document).on('click', '.change_status', function(e) {
    ...
    if (status == 'active') {
      var response = confirm("{{ __('backend.confirm_box_active_commission') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_deactive_commission') }}");
    }
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/commission/changeStatus' )!!}" + "/" + id + '/' + status,
  ...
  $(document).on('click', '.sendUserNotification', function(e) {
    ...
    if (title && message) {
      $.ajax({
        url: '{{ url("admin/commission / send_notification ") }}',
        dataType: 'json',
        data: values,
        type: 'POST',
        beforeSend: function() {
  ...
  $(document).on('click', '.view_btn', function(e) {
    ...
    $.ajax({
      url: '{{url("admin/commission/view")}}/' + id,
      dataType: 'html',
      success: function(result) {
  ...
  $(document).on('click', '.delete_staff', function(e) {
    ...
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/api/commission' )!!}" + "/" + id,
        success: function() {
