...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor">{{ __('Attribute Manager') }}</h4>
    </div>
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ __('Home') }}</a></li>
                <li class="breadcrumb-item active">{{ __('Attribute Manager') }}</li>
            </ol>
            @can('Attribute-create')
              <a href="{{ url('attribute/create') }}" class="btn btn-primary d-lg-block m-l-15" title="{{ __('Add Attribute') }}"  ><i class="fa fa-plus"></i> {{ __('Add Attribute') }}</a>
        ...
        <div class="card card-primary card-outline">
                ...
                @if($user_type != 4)
  								<div class="col-xl-3 col-lg-3 col-md-4 col-sm-6">
  									<div class="form-group" style="margin-right:5px">
  										<select name="main_category_id" id="main_category_id" onchange="getCategoryByIds()" class="form-control select2" multiple="multiple"  data-placeholder="Select Service" data-dropdown-css-class="select2-primary">
                ...
                @endif
						...
						<div class="table-responsive">
							<table id="listing" class="table table-responsive table-striped table-bordered" style="width:100%">
								<thead>
									<tr>
										<!-- <th>{{ __('Sr. no') }}</th> -->
										<th>{{ __('Service') }}</th>
...
<script>
  ...
  function getCategoryByIds() {
    ...
    if (user_type == 4) {
      var main_category_ids = "{{$main_category_id}}";

    } else {
      var main_category_ids = $('#main_category_id').val();
    }
...
ajax_datatable = $('#listing').DataTable({
    ...
    columns: [
      ...
      { data: 'attributes_name', name: 'attributes_name', orderable: false, searchable: false },
    ...
    rowCallback: function(row, data, iDisplayIndex) {  
      ...
      @if($user_type == 1)
        links += `<a href="{{url('attribute/edit')}}/${data.id}" title="Edit Details" class="btn btn-primary btn-xs" ><span class="fa fa-edit"></span></a>`;
      ...
      if(data.status === 1){
        ...
        @else
          if(data.added_by == loginUserId) {
            ...
            status += `<a href="#" data-product_id="${data.id}" title="Active Account" data-status="deactive" class=""><span class='label label-rounded label-success'>Active</span></a>`;
          }
        @endif
        ...
        @else
          if(data.added_by == loginUserId) {
            ...
            status += `<a href="#" data-product_id="${data.id}" title="Deactive Account" data-status="active" class=""><span class='label label-rounded label-warning'>Deactive</span></a>`;
          }
        @endif
      }
...
$(document).on('click','.change_status',function(e){
      ...
      if(status == 'active'){
        var response = confirm('Are you sure want to active this attribute?');
      }else{
        var response = confirm('Are you sure want to deactive this attribute?');
      }
      if(response){
        ...
        $.ajax({
          ...
          dataType:'json',
          url: "{!! url('attribute/changeStatus' )!!}" + "/" + id +'/'+status,
...
$("#add_images").on('submit',function(e){ 
  ...
  var _this=$(this); 
    ...
    $.ajax({
    url:'{{ url('attribute/add-more-images') }}/'+image_id,
    dataType:'json',
    data:formData,
    type:'POST',
    cache:false,
