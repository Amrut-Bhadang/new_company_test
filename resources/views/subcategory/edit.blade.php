...
<form method="PUT" action="{{ url('api/category/'.$category->id) }}" id="edit_role">
        ...
        @if($user_type != 4)
          <div class="row">
              ...
              <div class="form-group">
                ...
                <select name="main_category_edit_id" class="form-control" onchange="getEditCategory()" data-parsley-required="true" >
    ...
    <?php
      if(isset($category))
      {
          $langData = SubcategoryLang::where(['lang'=>$lang,'sub_category_id'=>$category->id])->first();  
      } ?>
...
$("#edit_role").on('submit',function(e){ 
  ...
  var _this=$(this); 
    ...
    $.ajax({
      url:'{{ url('api/subcategory/'.$category->id) }}',
      dataType:'json',
      data:formData,
      type:'POST',
      cache:false,
...
function getEditCategory() {
  var main_category_id = $('select[name=main_category_edit_id]').val();
  var category_id = "<?php echo $category->category_id; ?>";

  $.ajax({
     url:'{{url('subcategory/show_service_category')}}/'+main_category_id+'/'+category_id,
     dataType: 'html',
...
function getEditSubcategory() {
  var category_id = $('select[name=category_id]').val();
  var sub_category_id = "<?php echo $category->id; ?>";
