...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor">{{ __('backend.ContactUs_Manager') }}</h4>
    </div>
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ url('admin/') }}">{{ __('backend.home') }}</a></li>
                <li class="breadcrumb-item active">{{ __('backend.ContactUs_Manager') }}</li>
            </ol>
           <!--  @can('Content-create')
...
<div class="modal fade" id="editModal">
      ...
      <div class="modal fade" id="viewModal">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">{{__('backend.View_ContactUs')}}</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
...
ajax_datatable = $('#content_listing').DataTable({
    ...
    rowCallback: function(row, data, iDisplayIndex) {  
      ...
      var mobile = '';
      if(data.mobile == ''){
        mobile += `<span class=''>no data found</span>`;
      }else{
        mobile += `<span class=''>${data.country_code}-${data.mobile}</span>`;
      }
...
$(document).on('click','.change_status',function(e){
      ...
      if(status == 'active'){
        var response = confirm("{{__('backend.confirm_box_active_contact_us')}}");
      }else{
        var response = confirm("{{__('backend.confirm_box_deactive_contact_us')}}");
      }
      if(response){
        ...
        $.ajax({
          ...
          dataType:'json',
          url: "{!! url('admin/contact_us/changeStatus' )!!}" + "/" + id +'/'+status,
...
$(document).on('click','.view_btn',function(e){
    ...
    id = $(this).attr('data-content_id');
    $.ajax({
       url:"{{url('admin/contact_us/view')}}/"+id,
       dataType: 'html',
       success:function(result)
       {
