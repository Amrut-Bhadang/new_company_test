...
<div class="row page-titles">
  <div class="col-md-5 align-self-center">
    <div class="d-flex align-items-center">
      ...
      <h4 class="text-themecolor">{{ __('backend.Edit_amenity') }}</h4>
    </div>
  </div>
  <!-- <div class="col-md-5 align-self-center">
  ...
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"> {{ __('backend.Edit_amenity') }} </li>
      </ol>
...
<div class="container-fluid">
  ...
  <div class="content">
    <form method="PUT" action="{{ url('admin/api/amenities/'.$amenity->id) }}" enctype="" id="edit_amenity">
      @csrf
      <div class="col-md-12">
            ...
            <div class="container-fluid" style="margin-top:10px">
              ...
              @if($language)
                    ...
                    <?php
                      if(isset($amenity))
                      {
                          $langData = AmenityLang::where(['lang'=>$lang,'amenity_id'=>$amenity->id])->first();  
                      }
                  ?>
                <div class="col-md-6">
                    <div class="form-group">
                      <label class="control-label" for="name"> {{__('backend.amenity_Name')}} ({{__('backend.'.$lang)}})*</label>
                      <input type="text" name="name[{{$lang}}]" data-parsley-required="true" value="{{$langData->name}}" id="court_name" class="form-control" placeholder=" {{__('backend.amenity_Name')}}"  />
                    </div>
                </div>
                @endforeach
                  @endif
                ...
                <div class="col-md-12 mb-6">
                  <div class="form-group">
                    <div class="input-group">
                      <div id="image_preview"><img height="100" width="100" id="previewing" src="{{$amenity->image}}"></div>
...
<script>
  $(document).ready(function() {
    ...
    // });
    $('.select2').select2();
    $('#edit_amenity').parsley();

    $(document).on('submit', "#edit_amenity", function(e) {
      e.preventDefault();
      var _this = $(this);
      ...
      $('#group_loader').fadeIn();
      // var values = $('#edit_amenity').serialize();
      $.ajax({
        url: "{{ url('admin/api/amenities/'.$amenity->id) }}",
        dataType: 'json',
        data: formData,
        type: 'POST',
        cache: false,
