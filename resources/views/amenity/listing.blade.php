...
<div class="row page-titles">
  <div class="col-md-5 align-self-center">
    <h4 class="text-themecolor">{{ __('backend.Amenity_Manager') }}</h4>
  </div>
  <div class="col-md-7 align-self-center text-right">
    <div class="d-flex justify-content-end align-items-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('admin/') }}">{{ __('backend.home') }}</a></li>
        <li class="breadcrumb-item active">{{ __('backend.Amenity_Manager') }}</li>
      </ol>
      @can('Amenity-create')
      <a href="{{ url('admin/amenities/create') }}" class="btn btn-info d-none d-lg-block m-l-15" title="{{ __('backend.Add_amenity') }}"><i class="fa fa-plus"></i> {{ __('backend.Add_amenity') }}</a>
          ...
          <form method="POST" id="search-form" class="form-inline-sec" role="form">
            <div class="row">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                <div class="form-group">
                  <select name="amenity_status" id="amenity_status" class="form-control select2" data-placeholder="{{ __('backend.Select_amenity_status') }}" data-dropdown-css-class="select2-primary">
                    <option value="">--{{ __('backend.Select_amenity_status') }}--</option>
              ...
              <!-- <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                  <div class="form-group">
                    <select name="amenity_owner_name" id="amenity_owner_name" class="form-control select2" data-placeholder="{{ __('backend.Select_amenity_owner') }}" data-dropdown-css-class="select2-primary">
                        <option value="">--{{ __('backend.Select_amenity_owner') }}--</option>
              ...
              <!-- <div class="col-xl-2 col-lg-3 col-md-3 col-sm-4 mobile_cls">
                <div class="form-group">
                  <select name="amenity_id" id="amenity_id" class="form-control select2"  data-placeholder="Select Amenity" data-dropdown-css-class="select2-primary">
...
<div class="modal fade" id="add_modal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form method="POST" action="{{ url('admin/api/amenities') }}" id="add_staff">
        @csrf
        ...
        <div class="modal-body">
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label class="control-label" for="amenity_id">Amenity</label>
                <select name="amenity_id" id="amenity_id" class="form-control select2" data-placeholder="Select Amenity" data-dropdown-css-class="select2-primary">
                  <option value="">--Select Amenity--</option>
...
<div class="modal fade" id="notificationModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{ url('admin/amenities/send_notification') }}" id="sendUserNotification">
          @csrf
...
<div class="modal fade" id="viewModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">{{ __('backend.View_amenity') }}</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
    ...
    ajax_datatable = $('#staff_listing').DataTable({
      ...
      ajax: {
        ...
        data: function(d) {
          ...
          d.amenity_status = $('select[name=amenity_status]').val();
          d.amenity_owner_name = $('select[name=amenity_owner_name]').val();
      ...
      rowCallback: function(row, data, iDisplayIndex) {
        ...
        @can('Amenity-edit')
        links += `<a href="{{ url('admin/amenities/edit') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.Edit_Details') }}" class="btn btn-info btn-xs" ><span class="fa fa-edit"></span></a>`;
        @endcan
        // @can('Amenity-delete')
        ...
        // links += `<a href="{{ url('admin/amenities/manage_timing') }}/${data.id}" data-staff_id="${data.id}" title="{{ __('backend.timing') }}" class="btn btn-warning btn-xs" ><span class="fa fa-clock"></span></a>`;
        @endcan
        /*@can('Amenity-edit')
        links += `<a href="{{ url('admin/amenities/view_address').'/' }}${data.id}" data-staff_id="${data.id}" title="View Address" class="btn btn-danger btn-xs view_address" ><span class="fa fa-home"></span></a>`;
        @endcan
        @can('Amenity-edit')
        links += `<a href="{{ url('admin/amenities/transaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Transaction" class="btn btn-dark btn-xs view_transaction" ><span class="fa fa-credit-card"></span></a>`;
        @endcan
        @can('Amenity-edit')
        // links += `<a href="{{ url('admin/amenities/giftstransaction').'/' }}${data.id}" data-staff_id="${data.id}" title="View Gifts Transaction" class="btn btn-secondary btn-xs view_transaction" ><span class="fa fa-gifts"></span></a>`;
        ...
        links += `</div>`;
        total_order += `<a href="{{ url('admin/amenities/view_orders').'/' }}${data.id}" data-staff_id="${data.id}" class="view_order"><span class='label label-rounded label-primary'>${data.total_order}  Orders</span></a>`;
  ...
  $(document).on('click', '.change_status', function(e) {
    ...
    if (status == 'active') {
      var response = confirm("{{ __('backend.confirm_box_active_amenity') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_deactive_amenity') }}");
    }
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/amenities/changeStatus' )!!}" + "/" + id + '/' + status,
  ...
  $(document).on('click', '.sendUserNotification', function(e) {
    ...
    if (title && message) {
      $.ajax({
        url: '{{ url("admin/amenities/send_notification ") }}',
        dataType: 'json',
        data: values,
        type: 'POST',
        beforeSend: function() {
  ...
  $(document).on('click', '.view_btn', function(e) {
    ...
    $.ajax({
      url: '{{url("admin/amenities/view")}}/' + id,
      dataType: 'html',
      success: function(result) {
  ...
  $(document).on('click', '.delete_staff', function(e) {
    ...
    if (response) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/api/amenities' )!!}" + "/" + id,
        success: function() {
