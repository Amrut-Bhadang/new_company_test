        ...
        <div class="card-body">
          <form method="POST" id="search-form" class="form-inline-sec" role="form">
            <div class="row input-daterange form_s">
              ...
              <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 court_data_show_div">
                <select name="challenge_type" id="challenge_type" style="width:200px" class="form-control select2" data-placeholder="{{ __('backend.Select_Challenges') }}" data-dropdown-css-class="select2-primary">
                  <option value="">--{{ __('backend.Select_Challenges') }}--</option>
            ...
            <table id="staff_listing" class="table table-striped table-bordered" style="width:100%">
              <thead>
                <tr>
                  ...
                  <th> {{ __('backend.Payment_Received_Status') }} </th>
                  <th> {{ __('backend.Joiner_Payment_Status') }} </th>
...
<script>
    ...
    ajax_datatable = $('#staff_listing').DataTable({
      ...
      ajax: {
        ...
        data: function(d) {
          ...
          d.challenge_type = $('select[name=challenge_type]').val();
      ...
      columns: [{
        ...
        {
          data: 'payment_received_status',
          name: 'payment_received_status'
        },
        {
          data: 'joiner_payment_status',
          name: 'joiner_payment_status'
        },
        {
          data: 'order_status',
          name: 'order_status'
      ...
      rowCallback: function(row, data, iDisplayIndex) {
        ...
        var booking_type = '';
        var order_status = '';
        var payment_received_status = '';
        var joiner_payment_status = '';
        var payment_type = '';
        ...
        links += `<a href="#" data-staff_id="${data.id}" title="{{ __('backend.Delete') }}" data-is_deleted="1" class="change_is_delete"><span class='btn btn-warning btn-xs'><span class="fa fa-trash"></span></span></a>`;
        ...
        if (data.user_name == '') {
          user_name += `<span> ${data.country_code} - ${data.mobile}</span>`;
        } else {
          user_name += `<span> ${data.user_name} <br>(${data.country_code} - ${data.mobile}) <br> ${data.user_email}</span>`;
        }
        if (data.total_amount == '' || data.total_amount == null) {
          total_amount += `No Amount`;
        } else {
          total_amount += `<span> ${data.total_amount} {{__('backend.AED')}}</span>`;
        }
        if (data.paid_amount == '' || data.paid_amount == null) {
          paid_amount += `No Amount`;
        } else if(data.payment_received_status != 'Received'){
          paid_amount += `No Amount`;
        }
        else {
          paid_amount += `<span> ${data.paid_amount} {{__('backend.AED')}}</span>`;
        }
        ...
        if (data.admin_commission_amount == '' || data.admin_commission_amount == null) {
          ...
          admin_commission_amount += `<span> ${data.admin_commission_amount} {{__('backend.AED')}}</span>`;
        }
        ...
        if (data.order_status == '') {
          ...
          order_status += ` <div class="dropdown dropdown${data.id}">
                    <button class="btn btn-primary dropdown-toggle dropdown_order-toggle" type="button" id="dropdownMenuButton" data-id="${data.id}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{ __('backend.Pending') }}
                    </button>
                    <div class="dropdown-menu dropdownmenu${data.id}" aria-labelledby="dropdownMenuButton">
                      <a data-id="${data.id}" title="Prepare Order" onclick="change_status(this)" data-order_status="Accepted" class="dropdown-item  status_btn" href="javascript:void(0);">{{ __('backend.Accept') }}</a>
                      <a data-id="${data.id}" title="Cancel Order" onclick="change_status(this)" data-order_status="Cancelled" class="dropdown-item" href="javascript:void(0);">{{ __('backend.Cancel') }}</a>
                    </div>
                  </div>`;
        } 
        ...
        //  order status end
        //  payment status
        if (data.payment_received_status == '') {
          payment_received_status += `No payment_received_status`;
        }
        else if (data.payment_type == 'online') {
          payment_received_status += `<span class="text-capitalize"> {{__('backend.Received')}}</span>`;
        }
         else if (data.payment_received_status == 'Pending') {
          payment_received_status += ` <div class="dropdown dropdown_cash${data.id}">
                    <button class="btn btn-primary dropdown-toggle dropdown_cash-toggle" type="button" id="dropdownMenuButton" data-id="${data.id}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{ __('backend.Pending') }}
                    </button>
                    <div class="dropdown-menu dropdowncashmenu${data.id}" aria-labelledby="dropdownMenuButton">
                      <a data-id="${data.id}" title="Prepare Order" onclick="change_payment_status(this)" data-payment_received_status="Received" class="dropdown-item  status_btn" href="javascript:void(0);">{{ __('backend.Received') }}</a>
                      <a data-id="${data.id}" title="Cancel Order" onclick="change_payment_status(this)" data-payment_received_status="NotReceived" class="dropdown-item" href="javascript:void(0);">{{ __('backend.NotReceived') }}</a>
                    </div>
                  </div>`;
        } 
        else if (data.payment_received_status == 'Received') {
        ...
        else if (data.payment_received_status == 'NotReceived') {
          payment_received_status += `<span class="text-capitalize"> {{__('backend.NotReceived')}}</span>`;
        }
        else {
          payment_received_status += `<span class="text-capitalize"> ${data.payment_received_status}</span>`;
        }
        //  payment status end
         //  joiner payment status
         if (data.joiner_payment_status == '') {
          joiner_payment_status += `No joiner_payment_status`;
        }
        else if (data.booking_type == 'normal') {
          joiner_payment_status += `N/A`; 
        } 
       
         else if (data.joiner_payment_status == 'Pending') {
          joiner_payment_status += ` <div class="dropdown dropdown_joiner${data.id}">
                    <button class="btn btn-primary dropdown-toggle dropdown_joiner-toggle" type="button" id="dropdownMenuButton" data-id="${data.id}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{ __('backend.Pending') }}
                    </button>
                    <div class="dropdown-menu dropdownjoinermenu${data.id}" aria-labelledby="dropdownMenuButton">
                      <a data-id="${data.id}" title="Prepare Order" onclick="change_joiner_payment_status(this)" data-joiner_payment_status="Received" class="dropdown-item  status_btn" href="javascript:void(0);">{{ __('backend.Received') }}</a>
                      <a data-id="${data.id}" title="Cancel Order" onclick="change_joiner_payment_status(this)" data-joiner_payment_status="NotReceived" class="dropdown-item" href="javascript:void(0);">{{ __('backend.NotReceived') }}</a>
                    </div>
                  </div>`;
        } 
        else if (data.joiner_payment_status == 'Received') {
          joiner_payment_status += `<span class="text-capitalize"> {{__('backend.Received')}}</span>`;
        }
        else if (data.joiner_payment_status == 'NotReceived') {
          joiner_payment_status += `<span class="text-capitalize"> {{__('backend.NotReceived')}}</span>`;
        }
        else {
          joiner_payment_status += `<span class="text-capitalize"> ${data.joiner_payment_status}</span>`;
        }
        //  joiner payment status end
        ...
        $('td:eq(15)', row).html(links);
        $('td:eq(13)', row).html(order_status);
        $('td:eq(11)', row).html(payment_received_status);
        $('td:eq(12)', row).html(joiner_payment_status);
 ...
 //  change joiner payment status 
    function change_joiner_payment_status($this) {

    id = $($this).attr("data-id");
    status = $($this).attr("data-joiner_payment_status");
    if (status == 'Received') {
      var response = confirm("{{ __('backend.confirm_box_received_cash_joiner') }}");
    } else {
      var response = confirm("{{ __('backend.confirm_box_not_received_cash_joiner') }}");
    }
    // alert(id);
    // return false;
    if (response) {
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/orders/changeJoinerPaymentStatus' )!!}" + "/" + id + '/' + status,
        success: function(res) {
          if (res.status === 1) {
            window.location.reload();
            toastr.success(res.message);
...
//  change order status 
  ...
  function exportOrderData() {
    ...
    var challenge_type = $('select[name=challenge_type]').val();

    if (challenge_type) {
      link += 'challenge_type=' + challenge_type;
    }
  ...
  $(document).on('click', '.change_is_delete', function(e) {
    e.preventDefault();
    is_delete = $(this).data('is_deleted');
    if (is_delete == '1') {
      var response = confirm("{{ __('backend.confirm_box_is_deleted_yes') }}");
    } 
    if (response) {
      id = $(this).data('staff_id');
      $.ajax({
        ...
        dataType: 'json',
        url: "{!! url('admin/orders/delete_orders' )!!}" + "/" + id + '/' + is_delete,
        success: function(res) {
          if (res.status === 1) {
            toastr.success(res.message);
            ajax_datatable.draw();
        ...
        error: function(jqXHR, textIsFeatured, textIsFeatured) {
          console.log(jqXHR);
          toastr.error(jqXHR.is_featuredText)
        }
      });
    }
    return false;
  ...
  $(document).on('click', '.dropdown_order-toggle', function() {
    var id = $(this).data('id');
  ...
  $(document).on('click', '.dropdown_cash-toggle', function() {
    var id = $(this).data('id');
    $(this).attr('aria-expanded', 'true');
    $(".dropdown_cash" + id).addClass('show');
    $(".dropdowncashmenu" + id).addClass('show');
  });
  $(document).on('click', '.dropdown_joiner-toggle', function() {
    var id = $(this).data('id');
    $(this).attr('aria-expanded', 'true');
    $(".dropdown_joiner" + id).addClass('show');
    $(".dropdownjoinermenu" + id).addClass('show');
  });
