...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor">{{ __('backend.category_manager') }}</h4>
    </div>
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ __('backend.home') }}</a></li>
                <li class="breadcrumb-item active">{{ __('backend.category_manager') }}</li>
            </ol>
            @can('Category-create')
              <a href="#" class="btn btn-primary d-none d-lg-block m-l-15" title="{{ __('backend.add_title_category') }}" data-toggle="modal" data-target="#add_modal" ><i class="fa fa-plus"></i> {{ __('backend.add_category') }}</a>
              <a href="{{ url('/category/exportUsers/0') }}" class="btn btn-warning d-none d-lg-block m-l-15" title="{{ __('Export') }}"><i class="fa fa-download"></i> {{ __('Export') }}</a>
              @endcan
        </div>
    </div>
</div>
<!-- /.content-header -->
    ...
    <div class="row">
        ...
        <div class="card card-primary card-outline">
                <div class="card-body">
                    <div class="">
                       <form method="POST" id="search-form" class="form-inline-sec" role="form" style="margin-top:15px;">
							<div class="row input-daterange">
								...
								<div class="col-xl-3 col-lg-3 col-md-3 col-sm-6">
									<div class="form-group">
									  <select name="main_category_id" id="main_category_id" class="form-control select2" multiple="multiple"  data-placeholder="Select Main Category" data-dropdown-css-class="select2-primary">
...
<div class="modal fade" id="add_modal">
  <div class="modal-dialog modal-lg">
    ...
    <form method="POST" action="{{ url('api/category') }}" id="add_category">
    @csrf
...
<div class="modal fade" id="viewModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">{{ __('backend.view_food_category') }}</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
$(document).on('click','.change_status',function(e){
      ...
      if(response){
        ...
        $.ajax({
          ...
          dataType:'json',
          url: "{!! url('category/changeStatus' )!!}" + "/" + id +'/'+status,
