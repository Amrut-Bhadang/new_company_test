...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor">{{ __('Chef Staff Manage') }}</h4>
    </div>
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ __('Home') }}</a></li>
                <li class="breadcrumb-item active">{{ __('Chef Staff Manage') }}</li>
            </ol>
            @can('Chef-create')
              <a href="#" class="btn btn-primary d-none d-lg-block m-l-15" title="{{ __('Add Chef Staff') }}" data-toggle="modal" data-target="#add_modal" ><i class="fa fa-plus"></i> {{ __('Add') }}</a>
            @endcan
        </div>
    </div>
</div>
            ...
            <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        <label class="control-label" for="parent_chef_id">Chaf Manager *</label>
                        <select name="parent_chef_id" class="form-control" data-placeholder="Select Chef" style="width: 100%;" data-parsley-required="true" >
                          @foreach ($chefManager as $chef)
...
ajax_datatable = $('#chef_listing').DataTable({
    processing: true,
    serverSide: true,
    ajax: '{{ url('api/chef-staff-list/'.$chefManager[0]['id']) }}',
    columns: [
